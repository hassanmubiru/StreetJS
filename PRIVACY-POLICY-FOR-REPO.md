...
Cross-reference [`SECURITY-AUDIT-2026.md`](./SECURITY-AUDIT-2026.md) for the full incident record and remediation status. The repo's current posture (see [`.gitleaks.toml`](./.gitleaks.toml)) is that this on-disk path is gitignored and allowlisted, and the production publish pipeline signs from the `STREET_PLUGIN_SIGNING_KEY` GitHub secret — **not** from any file in the tree. The lesson stands regardless: a private key that reaches a public remote must be rotated, not just deleted.
