...
import { inspect, type FetchLike } from '../inspector.js';
...
function stubFetch(reply: { status: number; headers: Record<string, string>; body: string }): {
  fetch: FetchLike;
  calls: number;
} {
  const box = { calls: 0 };
    ...
    return {
      ...
      headers: {
        forEach(cb: (value: string, key: string) => void) {
          for (const [k, v] of Object.entries(reply.headers)) cb(v, k);
        },
      },
      async text() {
        return reply.body;
  ...
  return { fetch, get calls() { return box.calls; } } as { fetch: FetchLike; calls: number };
}
