...
import type { LlmClient, CompletionOptions } from './llm-client.js';
...
function estimateHistoryTokens(messages: Array<{ role: string; content: string }>): number {
...
function parseToolCall(content: string): ParsedToolCall | null {
  ...
  if (jsonBlockMatch?.[1]) {
    try {
      const parsed = JSON.parse(jsonBlockMatch[1].trim()) as { tool?: string; function?: string; name?: string; args?: unknown; arguments?: unknown; input?: unknown };
  ...
  if (rawJsonMatch) {
    try {
      const parsed = JSON.parse(rawJsonMatch[0]) as { tool: string; args?: unknown };
...
export class AgentExecutor {
   ...
   */
  async run(userMessage: string, ctx?: unknown): Promise<string> {
  ...
  private async _summarizeHistory(
    ...
    if (middleHistory.length === 0) return;

    const summaryRequest: CompletionOptions = {
      model: 'gpt-4o',
      messages: [
    ...
    for (const m of recentHistory) history.push(m);
  }
  ...
  private _emitStep(
    ...
    if (ctxObj['res'] && typeof (ctxObj['res'] as Record<string, unknown>)['write'] === 'function') {
      const res = ctxObj['res'] as { write(data: string): void };
