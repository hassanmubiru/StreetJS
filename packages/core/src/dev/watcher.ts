...
export class DevWatcher {
  ...
  private readonly watcherHandles: FSWatcher[] = [];
  ...
  async compile(): Promise<boolean> {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      tsc.stderr?.on('data', (chunk: Buffer) => {
      ...
      tsc.on('close', (code) => {
        ...
        if (success) {
          ...
          process.stderr.write(
            `[dev] Compilation failed (exit ${code ?? 'null'}) — keeping previous server alive.\n`,
          );
        }
        resolve(success);
      });
  ...
  async restartServer(): Promise<void> {
    ...
    child.on('exit', (code, signal) => {
      ...
      if (signal !== 'SIGTERM' && code !== 0) {
        process.stderr.write(
          `[dev] Server exited unexpectedly (code=${code ?? 'null'}, signal=${signal ?? 'none'}).\n`,
        );
      }
   ...
   */
  private killServerProcess(): Promise<void> {
