// Unit tests for the Google Cloud Storage driver submodule (task 29.2). Unlike
// the S3-compatible providers, GCS uses a bucket/file object model, so the
// driver maps the StorageDriver contract directly onto a purpose-built
// structural GcsClientLike (not the shared S3 base). These tests exercise the
// injected-client path with an in-memory GcsClientLike double (no
// `@google-cloud/storage` SDK, no external service), the StorageConfigError
// guard when no client is injected, the connect() guard when the SDK is absent,
// and the shared driver contract-conformance suite.
//
...
/**
 * A minimal in-memory GcsClientLike double. It stores the raw bytes plus the
 * content type and custom-metadata map exactly as the real GCS object model
...
function makeFakeClient() {
  ...
  return {
    ...
    async getMetadata(name) {
      ...
      return {
        size: String(obj.bytes.byteLength),
        contentType: obj.contentType,
        etag: `${name}-etag`,
        updated: new Date(FIXED_NOW).toISOString(),
        timeCreated: new Date(FIXED_NOW).toISOString(),
...
function makeDriver() {
  return createGoogleCloudStorageDriver(makeFakeClient(), { clock: fixedClock });
}

// ── name ──────────────────────────────────────────────────────────────────────

test("driver name is fixed to gcs", () => {
  assert.equal(makeDriver().name, "gcs");
});
...
test("put/get round-trips bytes and typed metadata over the injected client", async () => {
  ...
  const content = bytes("gcs object payload");
...
test("createdAt is preserved across overwrite while updatedAt advances", async () => {
  let now = 1000;
  const driver = createGoogleCloudStorageDriver(makeFakeClient(), { clock: () => now });
...
test("putStream/getStream round-trip through the injected client", async () => {
  ...
  const driver = makeDriver();
  const payload = bytes("streamed gcs payload");
...
test("connectGoogleCloudStorageDriver throws StorageConfigError when bucket is missing", async () => {
  await assert.rejects(
    () => connectGoogleCloudStorageDriver({ bucket: "" }),
    (err) => {
      assert.ok(err instanceof StorageConfigError);
      assert.equal(err.provider, "gcs");
      return true;
...
registerStorageDriverContractTests(
  "gcs",
  () => createGoogleCloudStorageDriver(makeFakeClient(), { clock: fixedClock }),
  test,
);
