...
| F-3 | High | Process gap: only `@streetjs/plugin-htmx` has a CI signing workflow. `@streetjs/plugin-marzpay@1.1.0` was signed during a **manual local publish** using the on-disk key, meaning the trust-root key reaches developer workstations. | Open | `sign-htmx.yml` is the only signing workflow; `packages/plugin-marzpay/manifest.signed.json` present. |
